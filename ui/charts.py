...
def valve_outlook_curve(
    ...
    *,
    review_year: float | None = None,
) -> go.Figure:
    """Compare this profile with its reference and mark reassessment timing."""
    ...
    figure.update_xaxes(title="Years from today", range=[0, float(t.max())], dtick=2, showgrid=False, zeroline=False, linecolor=GRID)
    figure.update_yaxes(title="Estimated probability that the valve remains free from failure", range=[0, 100], dtick=20, ticksuffix="%", showgrid=True, gridcolor=GRID, zeroline=False)
    return figure
